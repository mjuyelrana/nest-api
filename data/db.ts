...
import { Lesson } from "../types/lession";
...
export const findCourseById = (courseId: number): Course => findAllCourses().find((course: Course) => (course.id === courseId) ? course : {} as Course);
export const findLessonsForCourse = (courseId: number): Lesson => findAllLessons().find((lesson: Lesson) => (lesson.courseId === courseId) ? lesson : {} as Lesson);
export const authenticate = (email: string, password: string) => findAllUsers().find((user: User) => (user.email === email && user.password === password) ? user : {} as User);
